...
1. Eu uso o GitHub Actions para iniciar o workflow diariamente às 08:00 em `America/Sao_Paulo` ou manualmente por `workflow_dispatch`.
2. O arquivo `scripts/generate_brief.py` coleta apenas as URLs definidas em `config/sources.json`.
3. O Gemini recebe os itens coletados e retorna um relatório JSON estruturado.
4. O script escapa o conteúdo, gera `site/index.html` e preserva o último resultado válido se houver falha.
5. O GitHub Pages publica o diretório `site` como uma página web acessível remotamente.
...
1. Eu mantenho este projeto em um repositório público do GitHub.
2. Eu crio uma chave gratuita no [Google AI Studio](https://aistudio.google.com/apikey), sem ativar Cloud Billing.
3. Em `Settings > Secrets and variables > Actions`, eu cadastro essa chave no secret `GEMINI_API_KEY`.
4. Opcionalmente, eu configuro a variável `GEMINI_MODEL`; o padrão é `gemini-3.1-flash-lite`.
5. Em `Settings > Pages`, eu seleciono `GitHub Actions` como fonte de publicação.
6. Eu executo `Daily Cybersecurity Brief` manualmente uma vez para validar a configuração.

Eu uso o Gemini Free Tier para evitar custos de API. Não ativo o Cloud Billing no projeto Google para impedir a migração para a camada paga. O nível gratuito possui limites de taxa; quando a cota for excedida, o workflow preserva o último relatório válido.
...
Eu acompanho falhas pelas notificações do GitHub, conforme as preferências da minha conta. Para receber notificações de sucesso por Telegram, Discord ou e-mail transacional, posso adicionar um canal externo depois da primeira execução e guardar o webhook em GitHub Secrets.
